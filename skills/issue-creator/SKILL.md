...
If the user is creating an issue for another repository, keep the same draft-first, confirmation-before-create, and sensitive-info rules, but do not apply Sealos-specific products, environments, milestones, or Project fields unless the user explicitly asks for them.
