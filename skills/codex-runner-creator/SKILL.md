...
1. Run the bundled validator from this skill directory when available. It requires Python 3.11+ or another Python with `tomli` installed. After the skill is installed locally, that path is usually:

   ```bash
   python3 ~/.codex/skills/codex-runner-creator/scripts/validate_environment_toml.py .codex/environments/environment.toml
   ```
